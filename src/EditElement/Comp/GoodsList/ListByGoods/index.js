...
import './index.less'
const RNMsgChannel = require('react-native-webview-messaging')
...
export default class ListByGoods extends React.Component {
	...
	setIntervalTimerShops = shopsUrl => {
		//初始缓存基础数据
		postJSON(shopsUrl,{mallId:configData.mallId,categoryId:'',currentPage:1,pageSize:1000}).then(res=>{
			if(res.msg == 'success'){
				const list = res.data;
				localStorage.removeItem('RYShopsList')
		...
		this.timerShops = setInterval(()=>{
			...
			postJSON(shopsUrl,{mallId:configData.mallId,categoryId:catgIdPost,currentPage:1,pageSize:1000}).then(res=>{
				if(res.msg == 'success'){
					const list = res.data.data.list;
					this.setState({
						shops:list
	...
	getList = (page,fn) => {
		...
		categoryId ? shopsList = shopsList.filter(item=>{
			let list = item.categoryId,str = false;
			if(getAttr(list) == 'Array'){
				for(let i=0;i<list.length;i++){
					if(list[i] == parseInt(categoryId)){
						str = true
						break
					}
				}
	...
	//跳转页面
	toDetails = item => {
		const { animate, animateParams,page,data } = this.props,
