...
class ListByStore extends React.Component {
	...
	toDetails = item => {
		const { animate, animateParams,page,data,ioInput,ioOuter } = this.props,
		...
		if(ioInput.haveFloorMap){ 
			page ? Rymbp(page,item.recordId,203) : null;
		...
		return ( 
			<div
				...
				<div 
					...
					<p>{
						_.LOCAL_LOGO ? <img style={cssColorFormat(props, 'image')} src={'file://'+_.LOCAL_LOGO} /> : <img className="img_no_store" style={cssColorFormat(props, 'image')} src='./image/no_store.png' />
					}</p>
